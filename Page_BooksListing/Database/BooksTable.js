...
const booksData = [
    ...
    ['Karen M.McManus', 'One of us is back:Exclusiv e Edition - One of Us is Lying', 'Young Adult', 'Good', 12.00],
    ...
    ['Emma Torzas', 'Ink Blood Sister Scribe:Signe d Exclusive Edition', 'Fantasy', 'Like New', 18.00],
    ['Chimamanda Ngozi Adichie', 'Americanah', 'Contemporary Fiction', 'Very Good', 'Social Commentary', 13.50],
    ['Robert Louis Stevenson', 'Dr Jekyll and Mr Hyde- The Penguin English Library', 'Poor', 'Any book', 6.50],
...
function createTableAndInsertData() {
    ...
    db.run(createTableQuery, (err) => {
        if (err) {
            ...
            booksDictionary.forEach(book => { // <-- Fix the variable name here
                db.run(insertQuery, [book.author, book.title, book.genre, book.condition, book.trade_for, book.price, book.image], (err) => {
                    if (err) {
